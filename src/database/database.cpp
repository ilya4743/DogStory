...
void PlayerRecordRepositoryImpl::SaveRecordsTable(const std::vector<PlayerRecord>& player_records) {
...
std::vector<PlayerRecord> PlayerRecordRepositoryImpl::GetRecordsTable(size_t offset, size_t limit) {
    ...
    auto query_text = "SELECT name, score, play_time FROM hall_of_fame ORDER BY score DESC, play_time ASC, name ASC LIMIT " + std::to_string(limit) + " OFFSET " + std::to_string(offset) + ";";
...
Database::Database(const DbConnectrioSettings& db_settings)
    : connection_pool_{
